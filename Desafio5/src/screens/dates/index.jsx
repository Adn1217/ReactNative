...
import { Text, View, FlatList, ScrollView, KeyboardAvoidingView, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
import useOrientation from '../../hooks/useOrientation.jsx';
  ...
  const DatesScreen = ({route, navigation, dateList, setDateList}) => {
    ...
    const [dateListToShow, setDateListToShow] = useState(dateList.filter((item) => item.status === 'Pending'))
    ...
    function deleteItem(itemDeleted){
      let newDateList = dateList.filter((item) => item.id !== itemDeleted.id);
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={orientation === ORIENTATION.PORTRAIT ? styles.screenContainer : styles.screenContainerLandscape}>
            ...
            <View style={orientation === ORIENTATION.PORTRAIT ? styles.calendarContainer : styles.calendarContainerLandscape}>
              ...
              {dateListToShow.length > 0 ? <View style={orientation === ORIENTATION.PORTRAIT ? styles.flatListPortrait : styles.flatListContainer}>
                <Text style={orientation === ORIENTATION.PORTRAIT ? styles.title : styles.titleLandscape}>Citas pendientes</Text>
                <FlatList 
                  renderItem={dateToRender}
                  data={dateListToShow}
                  keyExtractor={(item) => item.id}
